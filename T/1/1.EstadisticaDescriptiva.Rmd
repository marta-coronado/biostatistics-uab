...
<tr>
  <td>Colonias bacterianas en una de cada seis placas de Petri</td>
  <td>Número de colonias</td>
  <td>Una placa de Petri</td>
</tr>
    </tr>
  </tbody>
</table>
...
- Peso de un bebé (kg)
- Concentración del colesterol en sangre (mg/dL)
...
--
<br><br>
> <i class="fa-solid fa-info-circle"></i> **Nota**: Una variable continua puede representarse tanto como **continua** como **categórica**, mientras que una variable categórica **solo puede ser categórica**.
...
La **escala ordinal** de variables corresponde a _variables categóricas_ que se pueden ordenar de una manera lógica/con significado.
...
- Variables continuas agrupadas en categorías (_binneadas_)
]
...
Las **escalas de intervalo** corresponden a variables continuas.
...
???
pH 0 es un punto arbitrario, no significa ausencia de pH
...
Las escalas de razón/proporción corresponden a variables continuas.
...
set.seed(123)
df_cont <- data.frame(
  peso = rnorm(1000, mean = 70, sd = 4)
)

p_continuo <- ggplot(df_cont, aes(x = peso)) +
  geom_histogram(binwidth = 0.2, fill = "#0E8A8A", alpha = 0.5) +
...
**Definición:**  
También llamada **promedio**, esta métrica corresponde al **valor central matemático** de los datos proporcionados.
...
.pull-left[
La media aritmética se calcula aplicando la función `mean()` de `R`.
...
.center[
$\text{moda}(x) = \max_{k=1}\left(I_{i=1}(x_i = x_k)\right)$
]
...
- La **moda** es más aplicable en **contextos de clasificación** y se usa con menor frecuencia, principalmente en variables cualitativas.
...
**Definición:**  
La varianza mide **cuánto se dispersan los valores de los datos respecto a su media**.
...
**Definición:**  
La desviación estándar cuantifica la **variación o dispersión** de un conjunto de datos.
...
Al calcular cuantiles, siempre se obtienen **un punto de corte menos** que el número de intervalos que se quieren generar.
...
--
La diferencia entre Q3 y Q1 es el **rango intercuartílico (IQR)**, que contiene el 50% central de los datos.
...
- Para mostrarlos en un **boxplot**, se calculan valores llamados **“cercas” (_fences_)**:  
