...
Ejemplo con la altura de personas adultas:
  ...
  - La media de altura en una muestra de 60 adultos es $\bar{x}_{altura} = 170 \text{ cm}$.
...
# creamos las medias muestrales para n = 1:1000 (1000 muestras, de 1, 2, ... 1000 individuos)
sample_sizes <- 1:1000
...
# 2. Tomamos 1000 muestras de tamaño 50 y calculamos la media de cada una
n <- 50
reps <- 1000
...
ggplot(df, aes(x = valor, fill = tipo)) +
  ...
  facet_grid(rows = vars(tipo), scales = "free_y") +
  labs(
    title = "Teorema central del límite: de una población sesgada a una distribución normal",
    x = "Valor / Media muestral",
    y = "Frecuencia"
  ) +
  scale_fill_manual(values = c("cadetblue4", "orange2")) +
...
- Aunque la población sea sesgada, la distribución de las medias muestrales tiende a ser aproximadamente normal cuando el tamaño de la muestra es suficientemente grande.
...
.pull-left[
- **Población**: media $\mu$ y desviación estándar $\sigma$
- **Muestra**: media $\bar{X}$ 
...
# 2. Tomamos 1000 muestras de tamaño 50 y de tamaño 500 y calculamos la media de cada una
n1 <- 50
n2 <- 500
...
En este ejemplo, ambas distribuciones son normales con media $\mu$, pero cambia la desviación estándar:
...
## Aplicación: probabilidades con la distribución muestral de la media

### Ejemplo 1: Duración de las bombillas 💡

Sabemos que la duración de las bombillas de una determinada marca sigue una distribución normal:  

$$
X \sim N(\mu = 1500, \sigma = 160)
$$

a) Queremos calcular la probabilidad de que una bombilla al azar funcione más de 1524 h.

b) Queremos calcular la probabilidad de que una muestra de 100 bombillas funcione en promedio más de 1524 h.
...
- Queremos calcular la probabilidad de que funcione más de 1524 h:  
$$
P(X > 1524)
$$

- Calculamos el valor $z$**, estandarizando**:  
$$
...
ggplot(df, aes(x=x, y=y)) +
  ...
  labs(title="Probabilidad de una bombilla > 1524 h",
...
$$
\bar{X} \sim N\left(\mu = 1500, \sigma_{\bar{X}} = \frac{\sigma}{\sqrt{n}} = \frac{160}{\sqrt{100}} = 16 \right)
$$

Queremos calcular la probabilidad de que la **media de la muestra** sea superior a 1524 horas:  

- Calculamos el valor $z$**, ahora con el error estándar porque trabajamos con la media**:

$$
z = \frac{\bar{x} - \mu}{\sigma / \sqrt{n}} = \frac{1524 - 1500}{160 / \sqrt{100}} = \frac{1524 - 1500}{16} = 1.5
$$
...
df <- data.frame(x, y)
 ...
 ggplot(df, aes(x=x, y=y)) +
  ...
  theme(panel.grid=element_blank(),
        axis.text=element_text(color="black"),
        panel.background = element_rect(fill = "white"))  + xlim(c(800,2300))
...
Considerando una muestra aleatoria simple de tamaño $n = 20$, queremos calcular la probabilidad de que el **peso medio** sea inferior a 550 g.
...
$$
\bar{X} \sim N\left(\mu = 600, \sigma_{\bar{X}} = \frac{\sigma}{\sqrt{n}} = \frac{100}{\sqrt{20}} \approx 22.36 \right)
$$
...
- Calculamos el valor $z$**, con el error estándar**:
$$
z = \frac{\bar{x} - \mu}{\sigma / \sqrt{n}} = \frac{550 - 600}{100 / \sqrt{20}} = \frac{550 - 600}{22.36} \approx -2.236
$$
...
.pull-left[
Sabemos calcular todo esto con la tabla de la distribución normal:
...
#### ✨Práctica! App para practicar el cálculo de probabilidades: 

[https://marta-coronado.shinyapps.io/app_normal/](https://marta-coronado.shinyapps.io/app_normal/)
...
Un **intervalo de confianza (IC)** es un rango de valores **simétrico respecto a la media muestral** que, con un determinado **nivel de confianza**, se utiliza para **estimar un parámetro poblacional desconocido**.
...
Si $\sigma$ es conocida se usa $\sigma$ en lugar de $s$. Si no, se usa $s$ y, con $n$ pequeño, el valor crítico sale de la t de Student (con $n$ grande, $z \approx t$).
...
- El área **fuera del intervalo** se llama $\alpha$.  
...
ggplot(df, aes(x=x, y=y)) +
  ...
  annotate("text", x=z_star+0.7, y=0.05, label="alpha/2", parse=TRUE) +
  # Texto central
  annotate("text", x=0, y=0.25, label=paste0(conf_level*100, "%"), fontface="bold", color="orange4") +
  labs(title="Distribución N(0,1): área central de dos lados",
...
.pull-left[
Queremos saber **entre qué dos valores de $z$ se encuentra el 95% central de la distribución normal estándar del estadístico $Z$ (no de los datos)** (los valores críticos del IC).
...
1. La **tabla Z** nos da el área acumulada **por debajo (o por encima) de un valor Z**.  
...
Esos valores $\pm z_{\alpha/2}$ son los que multiplican al error estándar en el IC: $\bar{x} \pm z_{\alpha/2}\cdot s/\sqrt{n}$.
]
...
Los valores críticos para el 95% son:
...
El 95% es la confianza del procedimiento: en el 95% de las muestras, el IC contiene a $\mu$.
 
---
class: animated, fadeIn

### Cálculo del IC al 90%
...
- De la tabla Z: $z_{\alpha/2} \approx 1.645$ (entre 1.64 y 1.65, que dan 0.9495 y 0.9505)  
- Intervalo de confianza:  
$$
IC_{90\%} = \bar{x} \pm 1.645 \cdot \frac{s}{\sqrt{n}}
$$
]
.pull-right[
...
### Valores críticos habituales (distribución normal)

> Para un IC bilateral. 

| Nivel de confianza | $\alpha$ | $z_{\alpha/2}$ |
|:-:|:-:|:-:|
| 80% | 0.20 | 1.28 |
| 90% | 0.10 | 1.645 |
| 95% | 0.05 | 1.96 |
...
<br>
> A mayor nivel de confianza, mayor $z_{\alpha/2}$ y, por tanto, un intervalo más ancho.
...
- Tomamos una **muestra aleatoria** de $n = 100$ personas y obtenemos una **media muestral**: $\bar{x} = 169 \text{ cm}$. Supongamos que $\sigma = 10 \text{ cm}$ es conocida.  
...
$$
IC = (\bar{x} - \text{m}, \bar{x} + \text{m})
$$

donde el **margen de error** se calcula como:

$$
\text{m} = z_{\alpha/2} \cdot \frac{\sigma}{\sqrt{n}}
$$

- $z_{\alpha/2}$: valor crítico según el nivel de confianza deseado  
- $\sigma$: desviación estándar poblacional (si es desconocida, usamos $s$) 
...
$x_1 = \bar{x} - z_{\alpha/2} \frac{\sigma}{\sqrt{n}}, \quad x_2 = \bar{x} + z_{\alpha/2} \frac{\sigma}{\sqrt{n}}$


---
class: animated, fadeIn

### Ejemplo: estatura media en Catalunya

**Datos:** $n = 100$, $\bar{x} = 169 \text{ cm}$, $\sigma = 10 \text{ cm}$ (conocida). Queremos un IC del 95%.

- Nivel de confianza: 95% → $z_{\alpha/2} = 1.96$

$$
\text{m} = z_{\alpha/2} \cdot \frac{\sigma}{\sqrt{n}} = 1.96 \times \frac{10}{\sqrt{100}} = 1.96
$$

$$
IC_{95\%} = (169 - 1.96,\ 169 + 1.96) = (167.04,\ 170.96)
$$

> Con un 95% de confianza, la **estatura media en Catalunya ($\mu$)** está entre **167.04 y 170.96 cm**.
...
```{r echo=FALSE, fig.height=8, fig.width=9, fig.align='center'}
library(ggplot2)
library(ggthemes)
set.seed(7)

# Población simulada (mu fijo, solo para la simulación)
mu <- 170; sigma <- 10; n <- 100
SE <- sigma / sqrt(n)
z_star <- 1.96
k <- 30   # número de muestras

x_bar <- rnorm(k, mean = mu, sd = SE)
df <- data.frame(muestra = 1:k, x_bar = x_bar,
                 lower = x_bar - z_star*SE, upper = x_bar + z_star*SE)
df$contiene <- ifelse(df$lower <= mu & df$upper >= mu, "Contiene μ", "No contiene μ")

ggplot(df, aes(y = muestra, x = x_bar, color = contiene)) +
    geom_linerange(aes(xmin = lower, xmax = upper), size = 1) +
    geom_point(size = 2) +
    geom_vline(xintercept = mu, linetype = "dashed") +
    scale_color_manual(values = c("Contiene μ" = "cadetblue4", "No contiene μ" = "firebrick3")) +
    labs(title = "30 intervalos de confianza del 95% (30 muestras distintas)",
         x = "Altura media (cm)", y = "Muestra", color = NULL) +
...
- El **margen de error $m$** disminuye:
...
- $z$ crítico: $z_{\alpha/2} = 1.645$  

$$
\text{m} = z \times \frac{\sigma}{\sqrt{n}} = 1.645 \times \frac{20}{\sqrt{250}} \approx 2.08
$$

$$
IC_{90\%} = (\bar{x} - \text{m}, \bar{x} + \text{m}) = (90 - 2.08, 90 + 2.08) \approx (87.92, 92.08)
$$


> Con un 90% de confianza, la **media poblacional ($\mu$)** está entre **87.92 y 92.08 minutos diarios**.
...
x_bar <- 90; sigma <- 20; z_star <- 1.645
df <- data.frame(n = c(25, 100, 250, 1000))
df$m <- z_star * sigma / sqrt(df$n)
df$lower <- x_bar - df$m
df$upper <- x_bar + df$m
df$n <- factor(paste0("n = ", df$n), levels = rev(paste0("n = ", df$n)))

ggplot(df, aes(y = n)) +
    geom_linerange(aes(xmin = lower, xmax = upper), size = 1.5, color = "cadetblue4") +
    geom_point(aes(x = x_bar), size = 3) +
    geom_vline(xintercept = x_bar, linetype = "dashed") +
    labs(title = "IC del 90% según el tamaño de la muestra",
         x = "Media de minutos diarios de actividad deportiva", y = NULL) +
    ggthemes::theme_base(base_size = 16) +
    theme(panel.grid = element_blank(),
          axis.text = element_text(color = "black"),
          panel.background = element_rect(fill = "white"))
...
- **Error máximo permitido**: $E = 1 \text{ min}$ 
...
$$
\text{m} = z_{\alpha/2} \cdot \frac{\sigma}{\sqrt{n}} \le E
$$
...
> Por lo tanto, necesitamos **al menos 1083 individuos** (redondeando hacia arriba) para garantizar que el error máximo en la estimación de $\mu$ sea menor o igual a 1 minuto con un 90% de confianza.
...
$$
n \ge \left(\frac{z_{\alpha/2} \times \sigma}{\text{error deseado}}\right)^2
$$
...
$$
(-\infty, \bar{x} + \text{m}) \quad \text{o} \quad (\bar{x} - \text{m}, \infty)
$$
...
Queremos construir un IC del **95%** de manera que podamos afirmar que, basándonos en la muestra, la media poblacional $\mu$ se encuentre **por debajo de** $\bar{x} + \text{m}$.  
...
De manera similar, que la media poblacional $\mu$ se encuentre **por encima de** $\bar{x} - \text{m}$.  

- Calculamos $z$ tal que el área en la **cola izquierda = 0.05** **(es decir, $z = -1.645$)** 

- Intervalo de confianza inferior: 

$$
(\bar{x} - 1.645 \times SE, \infty)
$$
...
Queremos comprobar el peso medio de los adultos en EEUU para dimensionar un equipamiento (ascensores, sillas, etc.). Solo nos interesa un límite superior: ¿hasta qué valor podemos asegurar que llega $\mu$?

**Datos:**  
- Muestra de 60 adultos de EEUU  
- Media muestral: $\bar{x} = 77 \text{ kg}$  
- Error estándar: $SE = 2.87 \text{ kg}$

**Cálculo del límite superior de un IC de 95% (*one-sided*):**  

$$
\text{límite superior} = \bar{x} + (1.645 \times SE) = 77 + (1.645 \times 2.87) \approx 81.72
$$

> En un IC unilateral todo el área $\alpha$ va en una sola cola, por eso usamos $z_{\alpha}$ (1.645 para el 95%) y no $z_{\alpha/2}$ (1.96).
...
> Con un 95% de confianza, el peso promedio de los adultos en EEUU se encuentra por debajo de 81.72 kg.
...
ggplot(df, aes(x=x, y=y)) +
  geom_line(size=1.2, color="black") +
  geom_area(data=subset(df, x <= z_star), aes(x=x, y=y), fill="cadetblue3", alpha=0.5) +
  geom_area(data=subset(df, x > z_star), aes(x=x, y=y), fill="firebrick3", alpha=0.4) +
  geom_vline(xintercept = z_star, linetype="dashed", color="firebrick3", size=1) +
  annotate("text", x=z_star+0.7, y=0.05, label="alpha", parse=TRUE) +
  annotate("text", x=0, y=0.25, label=paste0(conf_level*100, "%"), fontface="bold", color="orange4") +
  scale_x_continuous(breaks=c(-4, 0, z_star, 4), labels=c(-4, 0, round(z_star, 3), 4)) +
  labs(title="Distribución N(0,1): área de un solo lado",
       x=expression(z), y="Densidad") +
  ggthemes::theme_base(base_size = 13) +
  theme(panel.grid=element_blank(),
        axis.text=element_text(color="black"),
...
- No se debe decir que hay XX% de probabilidad de que el parámetro esté dentro del intervalo: $\mu$ es un valor fijo, y lo que varía de una muestra a otra es el intervalo. Una vez calculado, el intervalo contiene a $\mu$ o no lo contiene.
...
- Ojo: no es lo mismo que el intervalo que contiene al XX% de los valores individuales (por ejemplo, $\mu \pm 1.96\sigma$). Ese sí describe la población; el IC describe la incertidumbre sobre la media.
...
# mu fijo (solo para la simulación) y error estándar del ejemplo
mu <- 76.8
SE <- 2.87
z_star <- 1.645   # IC unilateral del 95%
k <- 30           # número de muestras

# Repetimos hasta que fallen 1 o 2 intervalos, para que se vea el caso "no contiene"
repeat {
  x_bar <- rnorm(k, mean = mu, sd = SE)
  if (sum(x_bar + z_star*SE < mu) %in% 1:2) break
}

x0 <- mu - 5*SE   # borde izquierdo (representa -Inf)
df <- data.frame(muestra = 1:k, x_bar = x_bar, upper = x_bar + z_star*SE)
df$contiene <- ifelse(df$upper >= mu, "Contiene μ", "No contiene μ")

ggplot(df, aes(y = muestra, color = contiene)) +
  geom_segment(aes(x = x0, xend = upper), size = 1,
               arrow = arrow(length = unit(0.15, "cm"), ends = "first")) +
  geom_point(aes(x = upper), size = 2) +
  geom_vline(xintercept = mu, linetype = "dashed") +
  scale_color_manual(values = c("Contiene μ" = "cadetblue4", "No contiene μ" = "firebrick3")) +
  coord_cartesian(xlim = c(x0, mu + 4*SE)) +
  labs(title = "30 IC unilaterales del 95% (30 muestras distintas)",
       x = "Peso medio (kg)", y = "Muestra", color = NULL) +
  ggthemes::theme_base(base_size = 13) +
  theme(panel.grid = element_blank(),
        axis.text = element_text(color = "black"),
        panel.background = element_rect(fill = "white"),
...
> Para un IC bilateral y unilateral. 
...
> Con el mismo nivel de confianza, el IC unilateral usa un $z$ menor, porque todo el $\alpha$ va en una sola cola.

<br>

> A mayor nivel de confianza, mayor $z$ y, por tanto, un intervalo más ancho
