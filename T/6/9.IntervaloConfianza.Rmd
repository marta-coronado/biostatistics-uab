...
.pull-left[
Queremos saber **entre qué dos valores de $z$ se encuentra el 95% central de la distribución normal estándar** (los valores críticos del IC).
...
$$
\text{error} = z_{\alpha/2} \cdot \frac{\sigma}{\sqrt{n}} = 1.96 \times \frac{10}{\sqrt{100}} = 1.96
$$
...
x_lower <- x_bar - z_star*SE
x_upper <- x_bar + z_star*SE

# Secuencia para curva normal de la media muestral
x <- seq(x_bar - 4*SE, x_bar + 4*SE, length=1000)
...
ggplot(df, aes(x=x, y=y)) +
    ...
    # Texto para alpha/2
  annotate("text", x=x_lower - 1, y=0.05, label="α/2") +
annotate("text", x=x_upper + 1, y=0.05, label="α/2") +
    # Texto central IC
    annotate("text", x=x_bar, y=0.15, label=paste0(conf_level*100, "% IC"), fontface="bold", color="orange4") +
    labs(title="Intervalo de confianza de dos lados",
         x="Altura media (cm)", y="Densidad") +
...
ggplot(df, aes(x=x, y=y)) +
    ...
    annotate("text", x=x_bar, y=max(y)*0.5, label=paste0(conf_level*100, "% IC"), fontface="bold", color="orange4") +
    labs(title="Intervalo de confianza de dos lados",
         x="Media de minutos diarios de actividad deportiva", y="Densidad") +
    ggthemes::theme_base(base_size = 16) +
    theme(panel.grid=element_blank(),
          axis.text=element_text(color="black"),
          panel.background = element_rect(fill = "white"))
...
De manera similar, que la media poblacional $\mu$ se encuentre **por encima de** $\bar{x} - \text{error}$.  
...
Queremos comprobar el peso medio de los adultos en EEUU para dimensionar un equipamiento (ascensores, sillas, etc.). Solo nos interesa un límite inferior: ¿a partir de qué valor podemos asegurar que se encuentra $\mu$?
...
> Con un 95% de confianza, el peso promedio de los adultos en EEUU se encuentra por encima de 72.28 kg.
