...
- Una variable aleatoria discreta puede tomar un número **finito o infinito numerable** de valores:
...
- Sin embargo, muchos conjuntos de datos provenientes de experimentos consisten en **mediciones continuas**. Una **variable aleatoria continua** puede tomar cualquier valor real dentro de un intervalo:
  - Altura en una población (cm)
  - Presión arterial en una población (mmHg)
...
```{r, fig.height=2.6, echo = F, fig.align='center'}
library(ggplot2)
...
p_brain <- ggplot(fat, aes(x = bicep)) +
  ...
  labs(title = "Circunferencia del bíceps extendido (cm)",
       x = "",
       y = "Densidad") +
  ggthemes::theme_base(base_size = 13) +
  theme(panel.grid=element_blank(),
...
p_normtemp <- ggplot(normtemp, aes(x = hr)) +
  ...
  labs(title = "Frecuencia cardíaca en reposo (lpm)",
       x = "",
       y = "Densidad") +
  ggthemes::theme_base(base_size = 13) +
  theme(panel.grid=element_blank(),
...
Una línea suave que representa una distribución de frecuencias se llama **curva de densidad**.
...
--
- Para dos números cualquiera _a_ y _b_, el área bajo la curva de densidad entre  _a_ y _b_ equivale a la proporción de valores _X_ entre  _a_ y _b_.
...
- **Está relacionada con** el **teorema central del límite**: la distribución **muestral** de la media, **con un tamaño de muestra suficientemente grande,** tiende a ser normal, aunque la población original no lo sea
...
???
- La altura de la curva es muy pequeña para valores de x a más de tres desviaciones estándar.
...
### Función de distribución acumulada normal en `R` 

- Por ejemplo: ¿cuál es la probabilidad de que $Z$ sea **mayor que -0.5**? y de que sea **menor que 1.42**?  

$$P(Z \ge -0.5), \quad \quad \quad P(Z \le 1.42)$$  

--

> En R usaríamos `pnorm()`: por defecto computa $P(Z \le z)$, la probabilidad acumulada _hasta_ $z$ para una distribución normal estándar $N(0,1)$:


```{r}
pnorm(q = 1.42, mean = 0, sd = 1)        # P(Z ≤ 1.42)
```

--
```{r}
1 - pnorm(-0.5, mean = 0, sd = 1)   # P(Z ≥ -0.5) = P(Z *≤ 0.5) = 1 - P(Z *≤ -0.5) 
```
...
<br>
- En la **distribución normal estándar**, $N(0,1)$::  
...
$$
X = 500 \times 2.33 + 3500
$$

$$
X \approx 1165 + 3500 = 4665 \, \text{g}
$$

Por lo tanto, un bebé en el percentil 99 pesaría aproximadamente **4665 g**.
...
<video  width="250" controls >
  <source src="https://github.com/marta-coronado/biostatistics-uab/raw/refs/heads/main/T/5/img/iq80.mp4" type="video/mp4" />
  Tu navegador no soporta la etiqueta de video.
</video>

</center>

Las pruebas modernas de IQ están calibradas de manera que el promedio se establece en 100, con una desviación estándar, generalmente de 15 puntos. **¿Es verdad que la mayoría de la población tiene un IQ menor de 80?**
...
En una población de arenque (*Alosa aestivalis*),  
...
> $$Y \sim N(54,\;4.5^2), \qquad P(58 \le X \le 60)\;=?$$
...
|----------------------|-------------------|----------------------------------------------|------------------------------------------------------|
| $P(X \le x)$         | “como mucho x”    | $P(X \le x)$                                 | `pnorm(x, mean = mu, sd = sigma)`                    |
| $P(X < x)$           | “menos de x”      | $P(X \le x)$ (igual, continua)               | `pnorm(x, mean = mu, sd = sigma)`                    |
| $P(X \ge x)$         | “al menos x”      | $1 - P(X \le x)$                             | `pnorm(x, mean = mu, sd = sigma, lower.tail = FALSE)`|
| $P(X > x)$           | “más de x”        | $1 - P(X \le x)$                             | `pnorm(x, mean = mu, sd = sigma, lower.tail = FALSE)`|
| $P(a \le X \le b)$   | “entre a y b”     | $P(X \le b) - P(X \le a)$                    | `pnorm(b, mu, sigma) - pnorm(a, mu, sigma)`          |
| $P(a < X < b)$       | “entre a y b”     | $P(X \le b) - P(X \le a)$ (igual, continua) | `pnorm(b, mu, sigma) - pnorm(a, mu, sigma)`          |

**Notas:**
- En la normal no importa la diferencia entre $<$ y $\le$ porque es continua.  
