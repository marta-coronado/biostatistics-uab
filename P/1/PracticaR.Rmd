...
<b><i class="fas fa-warning"></i>&nbsp;IMPORTANTE: </b> No has de instalar ninguna herramienta en los ordenadores de la facultad.</b> 
</div>
...
¡Ya estás listo/lista para empezar a usar `R`!
...
```{r, eval = F}
setwd("/ruta/nuevo/directorio")
```

O más sencillo, de manera gráfica dentro de RStudio a *Session > Set Working Directory > Choose Directory...*). 
...
<i class="fa fa-key"></i> Por ejemplo, puedes escoger una carpeta que hayas creado en el Escritorio o Descargas.
...
Lee en `R` el archivo ["**GSE68849_expression.csv**"](https://raw.githubusercontent.com/marta-coronado/biostatistics-uab/refs/heads/main/P/1/data/GSE68849_expression.csv). 
...
<i class="fa fa-key"></i> Puedes utilizar los comandos que vimos en la [diapositiva 12 de introducción a R](https://marta-coronado.github.io/biostatistics-uab/T/2/2.IntroR.html#18). Ten en cuenta que quizás necesites cargar una librería, como `library(readr)`, según la función que quieras usar.

<i class="fa fa-key"></i> No hace falta que descargues el fichero, `R` puede leerlo directamente con el enlace URL ["https://raw.githubusercontent.com/marta-coronado/biostatistics-uab/refs/heads/main/P/1/data/GSE68849_expression.csv"](https://raw.githubusercontent.com/marta-coronado/biostatistics-uab/refs/heads/main/P/1/data/GSE68849_expression.csv).

```{r, respuesta 2}

expressionData <- read.csv(file="https://raw.githubusercontent.com/marta-coronado/biostatistics-uab/refs/heads/main/P/1/data/GSE68849_expression.csv", header = TRUE)
...
Para saber el tipo de variable que contiene `expressionData`, podemos utilizar la función `str(expressionData)`. Rellena la siguiente tabla indicando qué tipo de variable contiene el `data.frame expressionData`.
...
Primero, podemos seleccionar con `select()` la columna `gene` de nuestro dataframe `expressionData` y luego usar `distinct()` para quedarnos solo con los valores únicos. Después, aplicamos `count()` para contar cuántos elementos hay en total.  
...
Una de las columnas de la tabla es `subject`. Extrae todos los datos correspondientes al sujeto `GSM1684096` y guárdalos en un nuevo objeto llamado `expressionData_GSM1684096`:
...
**Explora `expressionData`**. Estamos interesados en conocer el nivel de expresión del gen *IFNA5* en condición de infección para el sujeto `GSM1684104`. ¿Cómo lo harías? Combina `filter()` y `select()`:
...
Calcula la media del nivel de expresión para cada grupo (`treatment`). Deberás combinar `group_by()` y `summarize()`. La [diapositiva 43](https://marta-coronado.github.io/biostatistics-uab/T/2/2.IntroR.html#65) puede servir de guía:
...
Crea una nueva columna basada en el nivel de expresión de los genes: <10 → "bajo", 10-100 → "moderado", 100-1000 → "alto", ≥1000 → "muy alto"  (*binning*)

En la [diapositiva 32](https://marta-coronado.github.io/biostatistics-uab/T/2/2.IntroR.html#44) viste cómo hacerlo con `ifelse()` para dos grupos. Ahora lo haremos con `case_when()`, que permite trabajar con múltiples condiciones. Un ejemplo que puede servirte:
...
#### **<i class="fa fa-cogs"></i>** Actividad 1.11 (OPCIONAL)

En qué grupo de tratamiento están los genes con categoría de expresión `muy alto`? en los controles, en los de influenza, o ambos?
...
exp.heatmap <- exp.heatmap +
  labs(x = "???", y = "???") + 
  theme(axis.text.x = element_text(angle = ???, vjust = 0.5))
...
Entrega en el **Campus Virtual** el documento Rmd.
