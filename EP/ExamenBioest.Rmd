...
Se han realizado **tres ensayos independientes**. Los datos están disponibles en tres ficheros con formato `.csv` en los siguientes enlaces:

- Ensayo 1: https://raw.githubusercontent.com/marta-coronado/TAB-data-figs/refs/heads/main/dfA.csv
- Ensayo 2: https://raw.githubusercontent.com/marta-coronado/TAB-data-figs/refs/heads/main/dfB.csv
- Ensayo 3: https://raw.githubusercontent.com/marta-coronado/TAB-data-figs/refs/heads/main/dfC.csv

Los valores numéricos (columna `value`) representan el **número de semanas de supervivencia** de los ratones tras la administración del tratamiento (grupo tratamiento) o del placebo (grupo control). Los dos grupos de ratones son **independientes**.
...
**1. Un directivo (familiar de  Bradley Efron) le interesaría mucho hacer un Boostrap para uno de los ensayos, pero no tiene claro en qué ensayo sería más adecuado aplicarlo, si en el A o en el C. ¿Le puedes ayudar a elegir con la evidencia que tienes?** (0.25 puntos)
...
many_resampled_fireflies <- firefly  %>%   
  rep_sample_n(size = nrow(firefly), replace = FALSE, reps = 10000)

boot_diff <- many_resampled_fireflies %>%
  group_by(replicate) %>%
  summarise(
    diff_means = mean(subset(value, group == "Tratamiento")) -
                 mean(subset(value, group == "Control"))
  )
...
**3. Calculad el intervalo de confianza de la cola inferior al 95% de la diferencia de medias que has obtenido en `boot_diff`.** (0.5 puntos)
